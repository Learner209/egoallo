...
class MotionUNet(ModelMixin, nn.Module):
    ...
    def forward(
        ...
        decoder_out = (
            ...
            + self.encoders["body_rotmat"](x_t.body_rotmat.reshape(batch, time, -1))
            + self.encoders["contacts"](x_t.contacts)
        )
        
        if config.include_hand_motion and x_t.hand_rotmat is not None:
            decoder_out += self.encoders["hand_rotmat"](
                x_t.hand_rotmat.reshape(batch, time, -1)
            )
