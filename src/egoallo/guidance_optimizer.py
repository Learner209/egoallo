...
def do_guidance_optimization(
    ...
    params = traj.to_dict()
    optimizable_params = {
        k: torch.nn.Parameter(v, requires_grad=True) 
        for k, v in params.items()
    }
    
    optimizer = torch.optim.Adam([p for p in optimizable_params.values()], lr=learning_rate)
    ...
    for i in range(num_iters):
        ...
        if guidance_mode in ["hands", "full"]:
            hand_loss, hand_metrics = compute_hand_loss(
                body_model, current_traj, Ts_world_cpf, aria_detections
            )
        ...
        if guidance_mode == "full":
            body_loss, body_metrics = compute_full_body_loss(
                body_model, current_traj, Ts_world_cpf, hamer_detections
            )
...
def quaternion_to_matrix(quaternions: Float[Tensor, "... 4"]) -> Float[Tensor, "... 3 3"]:
