...
from third_party.humor.humor.body_model.body_model import BodyModel
from third_party.humor.humor.body_model.utils import SMPL_JOINTS, KEYPT_VERTS
from third_party.humor.humor.viz.utils import viz_smpl_seq
from third_party.humor.humor.utils.torch import copy2cpu as c2c
from third_party.humor.humor.utils.transforms import (
    batch_rodrigues,
    axisangle2matrots,
)

cur_file_path = os.path.dirname(os.path.realpath(__file__))
sys.path.append(os.path.join(cur_file_path, "third_party/humor/humor/"))
...
def determine_floor_height_and_contacts(body_joint_seq, fps):
    ...
    all_static_foot_heights = np.append(
        left_static_foot_heights,
        right_static_foot_heights,
    )
    ...
    left_heel_contact = np.logical_and(
        left_heel_contact,
        left_heel_heights < CONTACT_ANKLE_HEIGHT_THRESH,
    )
    right_heel_contact = np.logical_and(
        right_heel_contact,
        right_heel_heights < CONTACT_ANKLE_HEIGHT_THRESH,
    )
    left_toe_contact = np.logical_and(
        left_toe_contact,
        left_toe_heights < CONTACT_TOE_HEIGHT_THRESH,
    )
    right_toe_contact = np.logical_and(
        right_toe_contact,
        right_toe_heights < CONTACT_TOE_HEIGHT_THRESH,
    )
...
def process_seq(data_paths):
    ...
    # NOTE: this 05/05_07_poses.npz file is corrupted, skipping now.
    if input_file_path.find("05/05_07_poses.npz") >= 0:
        return
    ...
    joint_vel_seq = vtx_vel_seq = trans_vel_seq = root_orient_vel_seq = (
        pose_body_vel_seq
    ) = None
    if SAVE_VELOCITIES:
        h = 1.0 / fps
    ...
    print(
        "Processing: %s, Seq process time: %f s"
        % (input_file_path, time.time() - start_t),
    )
    return True
...
def main(config):
    ...
    # Using multiprocessing to process the sequences
    with Pool(config.num_processes) as pool:
        pool.map(process_seq, data_paths)
