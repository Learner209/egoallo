...
def mock_accelerator():
    ...
    accelerator.prepare = MagicMock(return_value=(MagicMock(), MagicMock(), MagicMock(), MagicMock()))
...
def test_eval_on_amass(tmp_path):
    ...
    config = InferenceConfig(
        checkpoint_dir=Path("/export/home/chenqixuan/code/muscle/egoallo/experiments/Jan_16_mr_92/v5/checkpoints_150000"),
...
# Test export_hdf5.py
@pytest.mark.parametrize("export_hdf5_cli_params", [
    {
        "data_npz_dirs": [
        ...
        "output_file": Path("./data/amass_rich_hps/processed_amass_rich_hps_correct.hdf5"),
        "output_list": Path("./data/amass_rich_hps/processed_amass_rich_hps_correct.txt")
    }
])
def test_export_hdf5(tmp_path, mock_body_model, test_params):
    from egoallo.scripts.export_hdf5 import main
    
    with patch("fncsmpl.SmplhModel.load", return_value=mock_body_model), \
...
@pytest.mark.parametrize("eval_cli", [
    ["python", "./src/egoallo/scripts/test.py",
     "--inference-config.dataset-slice-strategy", "full_sequence", 
     "--inference-config.splits", "test",
     "--inference-config.checkpoint-dir", "/export/home/chenqixuan/code/muscle/egoallo/experiments/combined_2_Jan_mr_0.75_foot_skating_loss/v0/checkpoints_690000",
     "--inference-config.dataset-type", "AdaptiveAmassHdf5Dataset",
     "--inference-config.visualize-traj",
     "--inference-config.debug-max-iters", "15"],  # Full command test case
    ["python", "./src/egoallo/scripts/test.py",
     "--inference-config.dataset-slice-strategy", "full_sequence", 
     "--inference-config.splits", "test",
     "--inference-config.checkpoint-dir", "/export/home/chenqixuan/code/muscle/egoallo/experiments/combined_2_Jan_mr_0.75_foot_skating_loss/v0/checkpoints_690000",
     "--inference-config.dataset-type", "AdaptiveAmassHdf5Dataset",
     "--inference-config.visualize-traj",
     "--inference-config.debug-max-iters", "15",
     "--inference-config.guidance-inner",
     "--inference-config.guidance-post"],  # Full command test case
    ["python", "./src/egoallo/scripts/test.py", 
     "--inference-config.dataset-slice-strategy", "full_sequence",
     "--inference-config.splits", "test", 
     "--inference-config.checkpoint-dir", "/export/home/chenqixuan/code/muscle/egoallo/experiments/combined_2_Jan_mr_0.75_foot_skating_loss/v0/checkpoints_690000",
     "--inference-config.dataset-type", "AriaDataset",
     "--inference-config.visualize-traj",
     "--inference-config.output-dir", "./exp/test-egoexo-train",
     "--inference-config.egoexo.split", "train",
     "--inference-config.guidance-mode", "no_hands",
     "--inference-config.debug-max-iters", "5",
     "--inference-config.guidance-inner",
     "--inference-config.guidance-post"],  # Full command test case
    ["python", "./src/egoallo/scripts/test.py", 
     "--inference-config.dataset-slice-strategy", "full_sequence",
     ...
     "--inference-config.egoexo.split", "train",
     "--inference-config.guidance-mode", "no_hands",
     "--inference-config.debug-max-iters", "5"],  # Full command test case
