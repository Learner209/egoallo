...
class SMPLViewer(BaseRenderer):
    ...
    def render_sequence(
        ...
        T_world_root = fncsmpl_extensions.get_T_world_root_from_cpf_pose(
            posed, data.T_world_cpf[..., 1:, :]
        )
        # breakpoint()
        global_root_orient_aa = SO3(T_world_root[..., :4]).log()
        pose = torch.cat(
            [
        ...
        with (
            ...
            for i in tqdm(range(T_world_root.shape[0] - 1), desc="Rendering frames"):
                ...
                T_world_cam = SE3(data.T_world_cpf) @ camera_offset
...
def visualize_ego_training_data(
    ...
    """
    viewer = SMPLViewer()
    viewer.render_sequence(data, denoised_traj, body_model, output_path)
