...
def signal_handler(signal_received, frame, logger, opt):
    ...
    if is_train:
        ...
        val_cfg_save_path = osp.join(opt.io.diffusion.val_save_path, f'{cfg_basename}_val.yaml')
        with open(val_cfg_save_path, 'w') as f:
            with redirect_stdout(f): print(val_cfg.dump())
        logger.info("Corresponding val config file has been saved to: {}".format(val_cfg_save_path))
...
def convert_to_dict(cfg_node, key_list=[]):
    ...
    if not isinstance(cfg_node, CN):
        if type(cfg_node) not in _VALID_TYPES:
            local_logger.warning("Key {} with value {} is not a valid type; valid types: {}".format(
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, epoch):
        ...
        if self.best_score is None:
            ...
            self.counter += 1
            local_logger.info(f'Epoch: [{epoch:4,d}], EarlyStopping counter: {self.counter} out of {self.patience}')
...
def images_to_video(img_folder, output_vid_file, ext='.png'):
    ...
    command = [
        'ffmpeg', '-r', '30', '-y', '-threads', '16', '-i', f'{img_folder}/%06d{ext}', '-profile:v', 'baseline',
