...
class EgoExoUtils:
    def __init__(self, opt, lazy_loading: bool = True, run_demo: bool = False):
        ...
        self._gt_bodypose_sample_ouput_dir = (
            self.preprocess_opt.gt_bodypose.sample_output.save_dir
        )
        self._aria_img_output_dir = self.preprocess_opt.aria_img_output_dir
        self._egoexo_train_data_output_dir = (
            self.preprocess_opt.egoexo_train_data_output_dir
        )
        ...
        gt_bodypose_output_dir = (
            self._gt_bodypose_output_dir
            if not run_demo
            else self._gt_bodypose_sample_ouput_dir
        )
        ...
        self.take_uid_to_take_names = {
            take["take_uid"]: take["take_name"] for take in self._all_takes
        }
        self.take_names_to_take_uid = {
            take["take_name"]: take["take_uid"] for take in self._all_takes
        }
        if not lazy_loading:
    ...
    def _load_splits_metadata(self):
        ...
        self.all_train_uids = self.egoexo_metadata["splits"]["split_to_take_uids"][
            "train"
        ]
        self.all_val_uids = self.egoexo_metadata["splits"]["split_to_take_uids"]["val"]
        self.all_test_uids = self.egoexo_metadata["splits"]["split_to_take_uids"][
            "test"
        ]
        self.all_take_uids = [
            take["take_uid"] for take in self.egoexo_metadata["takes"]
        ]
        self.all_take_names = [
            take["take_name"] for take in self.egoexo_metadata["takes"]
        ]
        ...
        self.all_train_takes = [
            self.find_take_from_take_uid(take_uid) for take_uid in self.all_train_uids
        ]
        self.all_val_takes = [
            self.find_take_from_take_uid(take_uid) for take_uid in self.all_val_uids
        ]
        self.all_test_takes = [
            self.find_take_from_take_uid(take_uid) for take_uid in self.all_test_uids
        ]

        # ! subsititute
        traj_dirs = [
            os.path.join(self.root_path, take["root_dir"], "trajectory")
            for take in self.egoexo_metadata["takes"]
        ]
        traj_dirs = list(
            filter(
        ...
        self.take_uids_to_benchmark = self.egoexo_metadata["splits"][
        ...
        self.all_take_uids_for_ego_hand_pose_benchmarks = set(
            [
                take_uid
    ...
    def get_egoexo_exported_data_output(self, anno_types, splits):
        ...
        for _, exported_data_path in enumerate(all_exported_data_paths):
            assert osp.exists(exported_data_path), (
                f"EgoExo Exported data path: {exported_data_path} does not exist"
            )
    ...
    def get_egoexo_exported_data_output_path(self, anno_types, splits):
        ...
        for _, anno_type in enumerate(anno_types):
            ...
            for _, split in enumerate(splits):
                ...
                export_egoexo_output_obj_path = os.path.join(
                    this_type_egoexo_train_data_output_dir,
                    f"egopose_gt_anno_{split}_public.p",
                )
    ...
    def get_all_uids_w_gt_anno(self, anno_type, split):
        ...
        # Load GT annotation
        if (
            anno_type not in self.raw_gt_anno_.keys()
    ...
    def get_take_metadata_from_take_uid(self, take_uid):
        ...
        take = [t for t in self.egoexo_metadata["takes"] if t["take_uid"] == take_uid][
            0
        ]
        open_loop_traj_path = (
            None
            if not self.has_open_loop_traj(take_uid)
            else self.load_open_loop_traj(take_uid)
        )
        close_loop_traj_path = (
            None
            if not self.has_close_loop_traj(take_uid)
            else self.load_close_loop_traj(take_uid)
        )
        gopro_calibs_path = (
            None
            if not self.has_gopro_calibs(take_uid)
            else self.load_gopro_calibs(take_uid)
        )
        cam_pose_anno_path = (
            None
            if not self.has_cam_pose_anno(take_uid)
            else self.load_cam_pose_anno(take_uid)
        )
        online_calib_json_path = (
            None
            if not self.has_online_calib_json(take_uid)
            else self.load_online_calib_json(take_uid)
        )
        vrs_path = None if not self.has_vrs(take_uid) else self.load_vrs(take_uid)
        no_image_streams_vrs_path = (
            None
            if not self.has_vrs_noimagestreams(take_uid)
            else self.load_vrs_noimagestreams(take_uid)
        )
        semidense_observations_path = (
            None
            if not self.has_semidense_observations(take_uid)
            else self.load_semidense_observations(take_uid)
        )
    ...
    def get_exported_mp4_path_from_take_name(self, take_name, exported_mp4_output_dir):
        ...
        """
        split = self.egoexo_metadata["splits"]["take_uid_to_split"][
            self.take_names_to_take_uid[take_name]
        ]
        mp4_output_root = os.path.join(exported_mp4_output_dir, "exported_mp4", split)
    ...
    def get_exported_mp4_path_from_take_uid(self, take_uid, gt_output_dir):
        ...
        mp4_output_root = os.path.join(gt_output_dir, "exported_mp4", split)
        mp4_file_path = osp.join(mp4_output_root, f"{take_name}.mp4")
        assert osp.exists(mp4_file_path), (
            f"mp4_file_path: {mp4_file_path} does not exist"
        )
    ...
    def get_undistorted_aria_imgs_from_take_uid(self, take_uid):
        ...
        aria_img_frame_nums = [
            int(osp.splitext(osp.basename(img))[0])
            for img in undistorted_aria_imgs_paths
        ]
    ...
    def find_take_uid_from_take_name(self, take_name):
        if take_name not in self.take_names_to_take_uid:
            return None
        else:
            return self.take_names_to_take_uid[take_name]

    def find_take_name_from_take_uid(self, take_uid):
        if take_uid not in self.take_uid_to_take_names:
            return None
        else:
    ...
    def has_cam_pose_anno(self, take_uid):
        ...
        """
        split = self.egoexo_metadata["splits"]["take_uid_to_split"][take_uid]
        self.take_uid_to_take_names[take_uid]
        cam_pose_anno_path = osp.join(
            self.root_path,
            "annotations",
            "ego_pose",
            split,
    ...
    def has_vrs_noimagestreams(self, take_uid):
        ...
        take = self.find_take_from_take_uid(take_uid)
        ego_cam_name = EgoExoUtils.get_ego_aria_cam_name(take)
        vrs_noimagestreams_path = osp.join(
            self.root_path,
            "takes",
            take_name,
            "{}_noimagestreams.vrs".format(ego_cam_name),
        )
    ...
    def has_semidense_observations(self, take_uid):
        ...
        semidense_observations_path = osp.join(
            self.root_path,
            "takes",
            take_name,
            "trajectory",
