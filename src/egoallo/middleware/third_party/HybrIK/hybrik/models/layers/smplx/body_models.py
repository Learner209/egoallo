...
class SMPL(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.register_buffer(
            'faces_tensor',
            to_tensor(
                to_np(self.faces, dtype=np.int64),
                dtype=torch.long,
            ),
        )
        ...
        if create_body_pose:
            if body_pose is None:
                ...
                if torch.is_tensor(body_pose):
                    ...
                    default_body_pose = torch.tensor(
                        body_pose,
                        dtype=dtype,
                    )
            self.register_parameter(
    ...
    def forward(
        ...
        **kwargs,
        ...
        global_orient = (
            global_orient if global_orient is not None else
            self.global_orient
        )
        ...
        batch_size = max(
            betas.shape[0], global_orient.shape[0],
            body_pose.shape[0],
        )
...
class SMPLH(SMPL):
    ...
    def create_mean_pose(self, data_struct, flat_hand_mean=False):
        ...
        body_pose_mean = torch.zeros(
            [self.NUM_BODY_JOINTS * 3],
            dtype=self.dtype,
        )

        pose_mean = torch.cat(
            [
                global_orient_mean, body_pose_mean,
                self.left_hand_mean,
    ...
    def forward(
        ...
        **kwargs,
        ...
        betas = betas if betas is not None else self.betas
        left_hand_pose = (
            left_hand_pose if left_hand_pose is not None else
            self.left_hand_pose
        )
        right_hand_pose = (
            right_hand_pose if right_hand_pose is not None else
