"""
A test on fncsmplh_aadecomp body model wrapper. There are unfinished jobs as the `SmplhModelAADecomp` is currently not used.
"""
import os
import sys
...
project_root = Path(__file__).parent.parent.parent.parent.parent.parent.parent.parent.parent.parent
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Test SMPLH twist function')
    ...
    parser.add_argument(
        ...
        help='Visualize the SMPLH model using Open3D',
    )
    parser.add_argument(
...
def init_smplh_model(model_path):
    """Initialize SMPLH model."""
    smpl_family_meta_model_name: SmplFamilyModelTypeLiteral = "SmplhModelAADecomp"
    smplh = SmplFamilyMetaModelZoo[smpl_family_meta_model_name].load(
                model_path,
    )
    return smplh

def init_smplx_model(model_path):
...
def process_frame(smplh: SmplFamilyModelType, smplx: SmplFamilyModelType, data, frame_idx, depth_factor):
    ...
    LEAF_JNT_NAMES = ['rthumb', 'rindex', 'rmiddle', 'rring', 'rpinky', 'lthumb', 'lindex', 'lmiddle', 'lring', 'lpinky']
    SMPLX_LEAF_VERTEX_INDEICES = [vertex_ids['smplx'][_] for _ in LEAF_JNT_NAMES]
    leaf_jnt_positions = torch.FloatTensor(data['pred_vertices'][frame_idx][0][SMPLX_LEAF_VERTEX_INDEICES]).unsqueeze(0)
    ...
    pred_xyz_jts = torch.cat([pred_xyz_jts, leaf_jnt_positions], dim=-2)
    ...
    padded_betas = torch.zeros(1, 16, device=pred_betas.device)
    padded_betas[:, :11] = pred_betas

    with torch.no_grad():
        output = smplh.model.hybrik(
            betas=padded_betas,
            pose_skeleton=pred_xyz_jts * depth_factor,  # scale to meters
            phis=cos_sin_phis[..., :51, :],  # use computed twist angles
            transl=None,
            return_verts=True,
            root_align=True,
        )
...
def main():
    ...
    # Initialize SMPLH model
    print(f"Initializing SMPLH model from {args.smpl_family_model_basedir}")
    smplh = init_smplh_model(Path(args.smpl_family_model_basedir))
    ...
    for frame_idx in range(n_frames):
        ...
        frame_result = process_frame(smplh, smplx, data, frame_idx, args.depth_factor)
        results.append(frame_result)

        # Print statistics
        vertices = frame_result['vertices']
        joints = frame_result['joints']
        faces = smplh.model.faces_tensor.numpy(force=True)
