...
def get_alpha(imtmp, bgval=1.):
    ...
    im_RGBA = cv2.merge((
        b_channel, g_channel, r_channel, alpha.astype(
        imtmp.dtype,
        ),
    ))
    return im_RGBA
...
def draw_skeleton(input_image, joints, draw_edges=True, vis=None, radius=None):
    ...
    for child in range(len(parents)):
        ...
        if draw_edges:
            cv2.circle(
                image, (point[0], point[1]), radius, colors['white'].tolist(),
                -1,
            )
            cv2.circle(
                image, (point[0], point[1]), radius - 1,
        ...
        if draw_edges and pa_id >= 0:
            ...
            cv2.circle(
                image, (point_pa[0], point_pa[1]), radius - 1,
                colors[jcolors[pa_id]].tolist(), -1,
            )
            if child not in ecolors.keys():
                print('bad')
            cv2.line(
                image, (point[0], point[1]), (point_pa[0], point_pa[1]),
