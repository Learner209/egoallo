...
class H36mSMPL(data.Dataset):
    ...
    def _load_jsons(self):
        ...
        if self._det_bbox_file is not None:
            bbox_list = json.load(
                open(
                    os.path.join(
                    self._root, 'annotations', self._det_bbox_file + f'_protocol_{self.protocol}.json',
                    ), 'r',
                ),
            )
    ...
    def evaluate_uvd_24(self, preds, result_dir):
        ...
        for n in range(sample_num):
            ...
            # prediction save
            pred_save.append({
                'image_id': int(image_id), 'joint_cam': pred_3d_kpt.tolist(
                ), 'bbox': bbox.tolist(), 'root_cam': gt_3d_root.tolist(),
