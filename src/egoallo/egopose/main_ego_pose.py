...
def save_stats_collector(stats_collector, output_path: str, split: str, anno_type: str) -> None:
...
def analyze_preprocessing_pipeline(args, output_path: str) -> None:
    ...
    """
    logger = setup_logger(output=output_path, name="preprocessing_analysis")
    ...
    for anno_type in args.anno_types:
        for split in args.splits:
            ...
            gt_anno_output_dir = osp.join(
                output_path, "annotation", anno_type
            ) if split in ["train", "val"] else osp.join(output_path, "annotation")
            ...
            if split in ["train", "val"]:
                ...
                if len(gt_anno.db) == 0:
                    logger.info(
                        "[Warning] No gt-anno is found in the local file."
                    )
                else:
                    with open(
                        osp.join(
...
def log_analysis_results(stats: Dict[str, Any], split: str, anno_type: str, logger) -> None:
    ...
    logger.info(f"Valid Takes: {stats['valid_takes']} ({stats['valid_takes']/stats['total_takes']*100:.2f}%)")
    logger.info(f"Total Frames: {stats['total_frames']}")
    logger.info(f"Valid Frames: {stats['valid_frames']} ({stats['valid_frames']/stats['total_frames']*100:.2f}%)")
    ...
    logger.info(f"├── Biomechanical Invalid: {percentages['biomechanical_invalid']:.2f}%")
    ...
    if 'keypoint_distribution' in stats:
        ...
        for idx, count in sorted_dist.items():
            ...
            logger.info(f"Keypoint {idx:2d} ({keypoint_name:12s}): {count:6d} occurrences ({percentage:5.2f}%)")
...
def create_body_gt_anno(args):
    ...
    gt_output_dir = args.gt_bodypose.output.save_dir if not args.gt_bodypose.run_demo else args.gt_bodypose.output_dir
...
def print_saved_stats(output_path: str, splits: List[str], anno_types: List[str]) -> None:
