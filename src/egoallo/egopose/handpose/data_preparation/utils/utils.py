...
def body_jnts_dist_angle_check(curr_body_pose3d):
    ...
    assert (
        len(kintree_table) == curr_body_pose3d.shape[0]
        and curr_body_pose3d.shape[1] == 3
    ), (
        f"Invalid input shape: {curr_body_pose3d.shape} or invalid kintree_table {len(kintree_table)}"
    )
