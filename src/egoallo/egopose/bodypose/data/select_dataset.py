...
def define_Dataset(dataset_opt: CN, opt: CN):
    ...
    if dataset_type in ['egoexo']:
        ...
        from egoallo.data.datasets.egoexo_diffusion_dataset import EgoExoDiffusionDataset as D
    ...
    logger.info('Dataset [{:s} - {:s}] is created.'.format(dataset.__class__.__name__, dataset_opt['phase']))
    return dataset
