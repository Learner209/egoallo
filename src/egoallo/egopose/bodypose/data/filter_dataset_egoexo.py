...
def load_raw_anno(opt, anno_type, split, run_demo=False):
    ...
    gt_anno_output_dir = osp.join(
        gt_bodypose_anno_output_dir, "annotation", anno_type
    )
...
class Filtered_Dataset_EgoExo(Dataset):
    def __init__(self, opt):
        ...
        for take_idx, take_uid in tqdm(enumerate(self.take_uids), total=len(self.take_uids), desc="Enumerating take uids", ascii=' >='):
            assert take_uid == self.anno[take_uid]["metadata"]["take_uid"]
            take_name = self.anno[take_uid]["metadata"]["take_name"]

            valid_frames = np.asarray(sorted(find_numerical_key_in_dict(self.anno[take_uid])))
            if len(valid_frames) < self.slice_window:
                self.skip_uids.append(take_uid)
                continue

            Ts_world_cam = np.stack([self.anno[take_uid][str(frame_idx)]["ego_camera_extrinsics"] for frame_idx in valid_frames],axis=0)

            body_3d = np.stack([self.anno[take_uid][str(frame_idx)]["body_3d"] for frame_idx in valid_frames],axis=0)
            body_valid_3d_flags = np.stack([self.anno[take_uid][str(frame_idx)]["body_valid_3d"] for frame_idx in valid_frames],axis=0)
    ...
    def __getitem__(self, index):
        ...
        body3d_valid_flags_win = self.body_3d_anno_valid_flags[take_uid][frames_window] # T x 17
...
class Filtered_Dataset_EgoExo_inference(Dataset):
    def __init__(self, opt):
        ...
        for take_idx, take_uid in tqdm(enumerate(self.take_uids), total=len(self.take_uids), desc="Enumerating take uids", ascii=' >='):
            ...
            Ts_world_cam = np.stack([self.anno[take_uid][str(frame_idx)]["ego_camera_extrinsics"] for frame_idx in valid_frames],axis=0) # T x 3 x 4
