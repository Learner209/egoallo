...
from egoallo.data.build import MultiEpochsDataLoader, _RepeatSampler
...
def inference(test_loader, model: ModelEgoExo4D, dry_run=False):
    ...
    for index, test_data in tqdm(enumerate(test_loader), total=len(test_loader), desc="Enumerating inference loader", ascii=' >='):
...
def test(test_loader, model: ModelEgoExo4D, epoch, current_step, test_step, dry_run=False):
    pos_error = []
    vel_error = []
    tasks = []
    inference_dict = {}
    ...
    for index, test_data in tqdm(enumerate(test_loader), total=len(test_loader), desc="Enumerating test loader", ascii=' >='):
        ...
        vel_visible = model.visible[0,1:,...].bool() & model.visible[0,:-1,...].bool()
        ...
        data_vel = torch.mean(torch.sqrt(torch.sum(torch.square(gt_vel-pred_vel),axis=-1)))
        ...
        inference_dict[test_data['take_uid'][0]]={"take_name":test_data['take_name'][0],"body":preds_}
        gt_dict[test_data['take_uid'][0]]={"take_name":test_data['take_name'][0],"body":gts_}
...
def main(opt):
    ...
    wandb.init(project=egopose_opt.wandb.project_name,
               config=egopose_opt,
    ...
    paths = (path for key, path in egopose_opt['path'].items() if 'pretrained' not in key)
    ...
    init_iter, init_path_G = option.find_last_checkpoint(egopose_opt['path']['models'], net_type='G')
    ...
    phases = [phase.lower() for phase in phases if phase in list(egopose_opt['datasets'].keys())]
    ...
    for phase in phases:
        ...
        if phase == 'train':
            ...
            logger.info('Number of train datum: {:,d}, iters: {:,d}'.format(len(train_set), train_size))
            train_loader = MultiEpochsDataLoader(train_set,
            ...
            test_loader = MultiEpochsDataLoader(test_set, batch_size=dataset_opt['dataloader_batch_size'],
    ...
    if egopose_opt['instantiate']['test']['dry_run'] and ('test' in phases or 'train' in phases):
    ...
    if "train" in phases:
        ...
        for epoch in range(100000):  # keep running
            for i, train_data in enumerate(train_loader):
                ...
                # merge bnorm
                # -------------------------------
                if egopose_opt['merge_bn'] and egopose_opt['merge_bn_startpoint'] == current_step:
                    logger.info('^_^ -----merging bnorm----- ^_^')
    ...
    if 'test' in phases:
        epoch = init_iter
        test_step = init_iter
        inference_dict, gt_dict = test(test_loader, model, epoch, current_step, test_step)
        dataset_opt = egopose_opt['datasets']['test']
        pred_path = os.path.join(egopose_opt['path']['images'],dataset_opt['split']+'_pred.json')
        gt_path = os.path.join(egopose_opt['path']['images'],dataset_opt['split']+'_gt.json')
