...
class EgoPoseDataPreparation:
    ...
    def align_exported_anno_to_slam_traj(
        self,
        take_uid,
        egoexo_util_inst: EgoExoUtils,
        this_take_valid_timestamps: NDArray,
        this_take_ego_cam_traj: NDArray,
        this_take_ego_cam_intr: NDArray,
        this_take_anno_3d: Optional[NDArray],
        ...
        (
            sampled_mps_close_traj_pose,
            T_world_imus_w_close_traj,
        ...
        (
            aligned_open_traj,
            aligned_open_traj_rot_mat,
            aligned_open_traj_quat_wxyz,
            _,
            _,
        ) = batch_align_to_reference_pose(
            sampled_mps_open_traj_pose[None,],
            sampled_mps_close_traj_pose,
        )
        aligned_open_traj, aligned_open_traj_rot_mat, aligned_open_traj_quat_wxyz = (
            aligned_open_traj[0],
            aligned_open_traj_rot_mat[0],
    ...
    def predict_pelvis_origin(self, this_take_anno_3d, this_take_anno_3d_valid_flag):
        ...
        assert J == len(BODY_JOINTS) + len(HAND_JOINTS) or len(BODY_JOINTS), (
            f"The input 3D annotation should have {len(BODY_JOINTS) + len(HAND_JOINTS)} or {len(BODY_JOINTS)} joints"
        )
        assert D == 3, "The input 3D annotation should have 3D coordinates"

        this_take_body_anno_3d, _this_take_hand_anno_3d = (
            this_take_anno_3d[:, : len(BODY_JOINTS)],
            this_take_anno_3d[:, len(BODY_JOINTS) :],
        )
        this_take_body_anno_3d_valid_flag, _this_take_hand_anno_3d_valid_flag = (
            this_take_anno_3d_valid_flag[:, : len(BODY_JOINTS)],
            this_take_anno_3d_valid_flag[:, len(BODY_JOINTS) :],
        )
        # T x (body_jnts), T x (hand_jnts)

        this_take_prev_body_anno_3d = (
        ...
        this_take_anno_3d_vel[0] = this_take_anno_3d_vel[
            1
        ]  # T x (body_jnts+hand_jnts) x 3
        body_vel_valid_flag = np.logical_and(
            ~np.isnan(np.mean(this_take_body_anno_3d, axis=-1)),
            ~np.isnan(np.mean(this_take_prev_body_anno_3d, axis=-1)),
        )  # T x (body_jnts+hand_jnts)
        ...
        tmp_body_anno_3d_w_left_ankle[miss_left_ankle_flag, left_ankle_idx] = (
            tmp_body_anno_3d_w_right_ankle[miss_left_ankle_flag, right_ankle_idx]
        )
        tmp_body_anno_3d_w_right_ankle[miss_right_ankle_flag, right_ankle_idx] = (
            tmp_body_anno_3d_w_left_ankle[miss_right_ankle_flag, left_ankle_idx]
        )

        this_take_root_height_3d = np.mean(
            np.stack(
                [
                    tmp_body_anno_3d_w_left_ankle[:, left_ankle_idx],
                    tmp_body_anno_3d_w_right_ankle[:, right_ankle_idx],
                ],
                axis=1,
            ),
            axis=1,
        ...
        for frame_ind in tqdm(
            ...
            this_take_body_anno_3d[frame_ind][
                this_take_anno_3d_valid_flag[frame_ind]
            ]  # J' x 3
            in_anno_3d_vel = this_take_anno_3d_vel[frame_ind][in_cluster_inds]  # J' x 3
            this_take_root_height_3d[frame_ind]  # J'
            ...
            for cur_label in all_labels:
                ...
                if VIZ_PLOTS:
                    plt.scatter(
                        cur_clust,
                        np.zeros_like(cur_clust),
                        label="foot %d" % (cur_label),
                    )
                ...
                # update min info
                min_median = min(cur_median, min_median)
    ...
    def extract_ankle_height(self, this_take_anno_3d, this_take_anno_3d_valid_flag):
        ...
        assert J == len(BODY_JOINTS) + len(HAND_JOINTS) or J == len(BODY_JOINTS), (
            f"The input 3D annotation should have {len(BODY_JOINTS) + len(HAND_JOINTS)} or {len(BODY_JOINTS)} joints, instead have {J}."
        )
        assert D == 3, "The input 3D annotation should have 3D coordinates"
    ...
    def predict_hip_trans(self, this_take_anno_3d, this_take_anno_3d_valid_flag):
        ...
        _this_take_body_anno_3d_valid_flag, _this_take_hand_anno_3d_valid_flag = (
            this_take_anno_3d_valid_flag[:, : len(BODY_JOINTS)],
            this_take_anno_3d_valid_flag[:, len(BODY_JOINTS) :],
        )
        # T x (body_jnts), T x (hand_jnts)
        ...
        this_take_trans_left_hip[miss_left_hip_flag] = this_take_trans_right_hip[
            miss_left_hip_flag
        ]
        this_take_trans_right_hip[miss_right_hip_flag] = this_take_trans_left_hip[
            miss_right_hip_flag
        ]
    ...
    def generate_smpl_converted_anno(
        self,
        this_take_valid_timestamps,
        this_take_anno_3d,
        this_take_anno_3d_valid_flag,
        ...
        **kwargs,
        ...
        _this_take_body_anno_3d, _this_take_hand_anno_3d = (
            this_take_anno_3d[:, : len(BODY_JOINTS)],
            this_take_anno_3d[:, len(BODY_JOINTS) :],
        )
        _this_take_body_anno_3d_valid_flag, _this_take_hand_anno_3d_valid_flag = (
        ...
        (
            this_take_root_height_3d,
            has_left_ankle_flag,
            has_right_ankle_flag,
        ) = self.extract_ankle_height(this_take_anno_3d, this_take_anno_3d_valid_flag)
        this_take_root_height_3d = (
            this_take_root_height_3d - CFG.empirical_val.ankle_floor_height_offset
        )  # T
        ...
        (
            T_smpl_world_cam,
            R_smpl_world_cam_as_euler,
            R_smpl_world_cam_as_rotvec,
        ) = openglpose2smplorigin(this_take_hip_traj)  # T x 4 x 4, T x 3, T x 3
        ...
        if VIS_W_GO3D:
            ...
            this_take_coco_model_gt = this_take_coco_model_gt[
                np.isin(coco_model_frames, this_take_valid_timestamps)
            ]  # T x 17 x 3
            this_take_coco_model_pred = this_take_coco_model_pred[
                np.isin(coco_model_frames, this_take_valid_timestamps)
            ]  # N x 17 x 3
            ...
            take_name = egoexo_util_inst.find_take_name_from_take_uid(take_uid)
            exported_mp4_file_path = (
                egoexo_util_inst.get_exported_mp4_path_from_take_name(
                    take_name=take_name,
                    exported_mp4_output_dir=self.export_mp4_root_dir,
                )
            )
            ...
            qpose_to_T(this_take_ego_cam_traj)

            exported_vrs_timestamps = aria_mps_serv.get_timestamps_from_exported_mp4()
            exported_vrs_timestamps = exported_vrs_timestamps[
                this_take_valid_timestamps
            ]
            T_mp4, *_ = exported_vrs_timestamps.shape
        ...
        for frame_ind in tqdm(
            range(T),
            total=T,
            desc="Geenrating smpl converted annotation using simple ik sovler",
            ascii=" >=",
        ):
            this_frame_smplh_anno_3d_valid_flag = this_take_smplh_anno_3d_valid_flag[
                frame_ind
            ]  # 52
            ...
            opt_local_aa_rep = simple_ik_solver_w_smplh(
            ...
            (
                opt_pred_smplh_jnts,
                opt_pred_smplh_verts,
                opt_pred_smplh_faces,
            ) = SMPLH_UTILS_INST.run_smpl_model(
                this_frame_hip_trans,
                opt_local_aa_rep,
                smplh_betas,
                self.bm,
                device=device,
            ...
            if VIS_W_MUJOCO:
                mujoco_assets_file = osp.join(
                    CFG.mujoco.mujoco_assets.root_path,
                    f"{CFG.mujoco.mujoco_assets.model_id}.xml",
                )
                smpl_m_viewer = SMPL_M_Viewer(model_file=mujoco_assets_file)
                T = 1
                smpl_local_aa_pose = (
                    opt_local_aa_rep[0, 0:1, :SMPLH_BODY_JNT_NUM, :]
                    .reshape(T, -1)
                    .detach()
                    .cpu()
                    .numpy()
    ...
    def convert_to_smplh_convention(
        ...
        J == len(BODY_JOINTS) + len(HAND_JOINTS)
        this_take_body_anno_3d, this_take_hand_anno_3d = (
            this_take_anno_3d[:, : len(BODY_JOINTS)],
            this_take_anno_3d[:, len(BODY_JOINTS) :],
        )
        _this_take_body_anno_3d_valid_flag, _this_take_hand_anno_3d_valid_flag = (
        ...
        this_take_smplh_body_anno_3d, this_take_smplh_hand_anno_3d = (
            this_take_smplh_anno_3d[:, : len(SMPLH_BODY_JOINTS)],
            this_take_smplh_anno_3d[:, len(SMPLH_BODY_JOINTS) :],
        )
        ...
        this_take_smplh_body_anno_3d[:, body_pose_valid_flag, :] = (
            this_take_body_anno_3d[
                :,
                np.asarray(EGOEXO4D_BODYPOSE_TO_SMPLH_INDICES)[body_pose_valid_flag],
                :,
            ]
        )
        ...
        if this_take_anno_has_hand_flag:
            ...
            this_take_smplh_hand_anno_3d[:, hand_pose_valid_flag, :] = (
                this_take_hand_anno_3d[
                    :,
                    np.asarray(EGOEXO4D_HANDPOSE_TO_SMPLH_w_HAND_INDICES)[
                        hand_pose_valid_flag
                    ],
                    :,
                ]
            )
        ...
        this_take_smplh_anno_3d_valid_flag = np.concatenate(
            [
                this_take_smplh_body_anno_3d_valid_flag,
                this_take_smplh_hand_anno_3d_valid_flag,
            ],
            axis=1,
        )
