...
from egoallo.utils.ego_geom import batch_align_to_reference_pose, aria_camera_device2opengl_pose
...
from egoallo.utils.aria_utils.open3d_vis import build_cam_frustum_w_extr as build_cam_frustum_w_extr_open3d
from egoallo.utils.aria_utils.open3d_vis import draw_coco_kinematic_tree as draw_coco_kinematic_tree_open3d
from egoallo.utils.aria_utils.open3d_vis import draw_camera_pose as draw_camera_pose_open3d
from egoallo.utils.aria_utils.ps_vis import build_cam_frustum_w_extr as build_cam_frustum_w_extr_ps
from egoallo.utils.aria_utils.ps_vis import draw_coco_kinematic_tree as draw_coco_kinematic_tree_ps
from egoallo.utils.aria_utils.ps_vis import draw_camera_pose as draw_camera_pose_ps
from egoallo.utils.aria_utils.plotly_vis import build_cam_frustum_w_extr as build_cam_frustum_w_extr_plotly
from egoallo.utils.aria_utils.plotly_vis import draw_coco_kinematic_tree as draw_coco_kinematic_tree_plotly
from egoallo.utils.aria_utils.plotly_vis import draw_camera_pose as draw_camera_pose_plotly
...
class AriaMPSService:
    ...
    def sample_close_loop_traj_from_timestamps(self, timestamps: NDArray, subsample_rate: int = 1) -> Tuple[NDArray, NDArray, NDArray, NDArray]:
        ...
        -----
         ...
         """
        if not isinstance(self.closed_loop_traj_path, str) or not osp.exists(self.closed_loop_traj_path):
            logger.warning(f"close loop trajectory file not found at {self.closed_loop_traj_path}")
        ...
        for sample_idx, sample_timestamp in enumerate(timestamps):
            sampled_loop_traj.append(get_nearest_pose(closed_loop_traj, sample_timestamp))

        sampled_loop_traj = sampled_loop_traj[::subsample_rate]

        sampled_close_loop_traj_pose = np.empty([len(sampled_loop_traj), 7])
        T_world_imus = np.tile(np.eye(4)[np.newaxis, :, :], (len(sampled_loop_traj), 1, 1))
        ...
        for ind, close_loop_pose in enumerate(sampled_loop_traj):
            ...
            quat_world_device = close_loop_pose.transform_world_device.rotation().to_quat()
        ...
        return sampled_close_loop_traj_pose, T_world_imus, close_timestamp_tracking_s, close_utc_timestamp

    def sample_open_loop_traj_from_timestamps(self, timestamps: NDArray, subsample_rate: int = 1) -> Tuple[NDArray, NDArray, NDArray, NDArray]:
        """Read open loop traj.
        ...
        -----
            ...
            open_utc_timestamp : numpy.ndarray of shape (N,)
                the UTC timestamp in seconds
        """
        if not isinstance(self.open_loop_traj_path, str) or not osp.exists(self.open_loop_traj_path):
            logger.warning(f"open loop trajectory file not found at {self.open_loop_traj_path}")
            return None, None, None, None

        opend_loop_traj = mps.read_open_loop_trajectory(self.open_loop_traj_path)
        sampled_loop_traj = []
        for sample_idx, sample_timestamp in enumerate(timestamps):
            sampled_loop_traj.append(get_nearest_pose(opend_loop_traj, sample_timestamp))
        ...
        for ind, open_loop_pose in enumerate(sampled_loop_traj):
            ...
            quat_world_device = open_loop_pose.transform_odometry_device.rotation().to_quat()
        ...
        return sampled_open_loop_traj_pose, T_world_imus, open_timestamp_tracking_s, open_utc_timestamp
    ...
    def read_open_loop_traj(self, skip : int = 100) -> Tuple[NDArray, NDArray, NDArray, NDArray]:
        """Read open loop traj.
        ...
        return mps_open_traj_subset, T_world_imus, open_timestamp_tracking_s, open_utc_timestamp

    def read_close_loop_traj(self, skip : int = 100) -> Tuple[NDArray, NDArray, NDArray, NDArray]:
        ...
        """
        if not isinstance(self.closed_loop_traj_path, str) or not osp.exists(self.closed_loop_traj_path):
            logger.warning(f"Closed loop trajectory file not found at {self.closed_loop_traj_path}")
            return None, None, None, None
        ...
        for ind, closed_loop_pose in enumerate(closed_loop_traj):
            ...
            quat_world_device = closed_loop_pose.transform_world_device.rotation().to_quat()
        ...
        return mps_close_traj_subset, T_world_imus, close_timestamp_tracking_s, close_utc_timestamp

    def get_exo_cams_calibs_w_cam_json(self, take : dict) -> Tuple[List[str], Dict[str, NDArray], Dict[str, NDArray]]:
        ...
        """
        if not isinstance(self.cam_pose_anno_path, str) or not osp.exists(self.cam_pose_anno_path):
            logger.warning(f"Camera pose annotation file not found at {self.cam_pose_anno_path}")
        ...
        for ind, exo_cam_name in enumerate(exo_cam_names):
            if not exo_cam_name in cam_pose_anno.keys():
                continue
            ...
            tmp_pose[:,3:] = transforms.matrix_to_quaternion(torch.from_numpy(T_exo_world_cam[:3, :3])).cpu().numpy()
            exo_cams_calibs_pose[exo_cam_name] = tmp_pose
        return exo_cam_names, exo_cams_calibs, exo_cams_calibs_pose

    def get_ego_cam_calibs(self, take : dict) -> Tuple[str, Dict[str, NDArray], Dict[str, NDArray]]:
        ...
        frames = find_numerical_key_in_dict(cam_pose_anno[ego_cam_name]["camera_extrinsics"])


        for frame_ind, frame_num in enumerate(frames):
            T_ego_cam_world = np.eye(4)
            T_ego_cam_world[:3] = cam_pose_anno[ego_cam_name]["camera_extrinsics"][str(frame_num)]
            ...
            tmp_pose[:,3:] = transforms.matrix_to_quaternion(torch.from_numpy(T_ego_world_cam[:3, :3])).cpu().numpy()
    ...
    def get_exo_cams_calibs_w_goprocalibs(self, take):
        ...
        exo_cams_calibs = {}
        exo_cams_calibs_pose = {}
        if not isinstance(self.goprocalibs_path, str) or not osp.exists(self.goprocalibs_path):
            logger.warning(f"GoPro calibration file not found at {self.goprocalibs_path}")
            return None, None, None
        static_calibrations = mps.read_static_camera_calibrations(self.goprocalibs_path)

        for ind, (static_calibration, exo_cam_name) in enumerate(zip(static_calibrations, exo_cam_names)):
            ...
            quat_world_device = static_calibration.transform_world_cam.rotation().to_quat()
    ...
    def plot_3d(self, T_world_imus_w_open=None,\
        T_world_imus_w_close=None, tracking_timestamps=None, valid_timestamps: NDArray =None, offset_z: NDArray = None, point_cloud=None, load_exo_cams=False, load_ego_cam_traj=False, **kwargs):
        """Plot elements.
        ...
        """
        if load_exo_cams:
            exo_cam_names, T_exo_world_cam_poses_w_cam_anno, T_exo_world_cam_qpose = self.get_exo_cams_calibs_w_cam_json(self.take)
            T_exo_world_cam_poses_w_cam_anno = {k: v[:3,:] for k, v in T_exo_world_cam_poses_w_cam_anno.items()}
        else:
            T_exo_world_cam_poses_w_cam_anno = None

        if load_exo_cams:
            exo_cam_names_, T_exo_world_cam_w_gopro_calib, T_exo_world_cam_qpose = self.get_exo_cams_calibs_w_goprocalibs(self.take)
            assert exo_cam_names_ == exo_cam_names, "Exo cam names do not match."
            T_exo_world_cam_w_gopro_calib = {k: v[:3,:] for k, v in T_exo_world_cam_w_gopro_calib.items()}
        else:
            T_exo_world_cam_w_gopro_calib = None
    
        if load_ego_cam_traj:
            ego_cam_name, T_ego_world_cams, T_ego_world_cam_poses = self.get_ego_cam_calibs(self.take)
            T_ego_world_cams = {k: v[:3,:] for k, v in T_ego_world_cams.items() if int(k) in valid_timestamps.tolist()}
            ...
            if T_world_imus_w_close is not None and T_world_imus_w_open is not None:
                ...
                assert T_ego == T_open == T_close, f"Open and close trajectory lengths do not match. Open: {T_open}, Close: {T_close}, Ego: {T_ego}"
        else:
            T_ego_world_cams = None

        self._plot_polyscope(T_exo_world_cam_poses_w_cam_anno=T_exo_world_cam_poses_w_cam_anno, 
    ...
    # region
        if not osp.exists(self.online_calib_json_path):
            raise ValueError(f"Online calibration json file not found at {self.online_calib_json_path}")
    ...
    def read_semidense_point_cloud(self):
        if not osp.exists(self.point_observation_path):
            raise ValueError(f"Point observation file not found at {self.point_observation_path}")
    ...
    def _plot_polyscope(self, T_exo_world_cam_poses_w_anno=None,
                T_exo_world_cam_w_gopro_calib=None,
                T_ego_world_cams=None, 
        ...
        ps.look_at_dir(camera_location=(0., -5., 2.5), target=(0., -1.5,-0.5), up_dir=(0.,0.,1.), fly_to=False)

        # Create a camera view from parameters
        intrinsics = ps.CameraIntrinsics(fov_vertical_deg=60, aspect=2)
        extrinsics = ps.CameraExtrinsics(root=(0., -2., 1.), look_dir=(0., 0.,0.), up_dir=(0.,0.,1.))
        ...
        if T_exo_world_cam_poses_w_anno is not None and CFG.plotly.camera_frustum.enable:
            for exo_cam_name in T_exo_world_cam_poses_w_anno.keys():
                T_exo_world_cam_pose_w_anno = T_exo_world_cam_poses_w_anno[exo_cam_name].copy()
                T_exo_world_cam_pose_w_anno[2, 3] += mean_offset_z
                build_cam_frustum_w_extr_ps(T_exo_world_cam_pose_w_anno, surface_mesh_id="exo_world_cam")

        if T_exo_world_cam_w_gopro_calib is not None and CFG.plotly.camera_frustum.enable:
            for exo_cam_name in T_exo_world_cam_w_gopro_calib.keys():
                ...
                build_cam_frustum_w_extr_ps(T_tmp, surface_mesh_id="exo_world_cam_gopro")

        if point_clouds is not None and CFG.plotly.semidense_observations.enable:
            threshold_invdep = 5e-4
            threshold_dep = 5e-4
            point_clouds = filter_points_from_confidence(point_clouds, threshold_invdep, threshold_dep)
            ...
            cloud.set_radius(0.0009) # radius is relative to a scene length scale by default
            cloud.set_material("candy")
            cloud.set_transparency(0.95)

        if T_world_imus_w_open is not None and T_world_imus_w_close is not None and CFG.plotly.closed_slam_traj.enable and CFG.plotly.open_slam_traj.enable:
            T_open, *_ = T_world_imus_w_open.shape
            T_close, *_ = T_world_imus_w_close.shape
            assert T_open == T_close == T, f"Open and close trajectory lengths do not match. Open: {T_open}, Close: {T_close}, T: {T}"
        ...
        if coco_world_3d_pts is not None and CFG.plotly.coco_kinematic_tree.enable:
            assert T == len(coco_world_3d_pts), f"Tracking timestamps and coco world 3d points do not match. Tracking: {T}, Coco: {len(coco_world_3d_pts)}"
        ...
        if model_pred_coco_kpts is not None and CFG.plotly.model_pred_coco_kpts.enable:
            assert T == len(model_pred_coco_kpts), f"Tracking timestamps and coco world 3d points do not match. Tracking: {T}, Coco: {len(model_pred_coco_kpts)}"
        ...
        if model_gt_coco_kpts is not None and CFG.plotly.model_gt_coco_kpts.enable:
            assert T == len(model_gt_coco_kpts), f"Tracking timestamps and coco world 3d points do not match. Tracking: {T}, Coco: {len(model_gt_coco_kpts)}"

        # Loading dynamic scenes
        for render_ind, tracking_timestamp in tqdm(enumerate(tracking_timestamps), total=min(T, save_max_frames), desc="enumerating polyscope routine rendering", ascii=' >='):
            ...
            if T_ego_world_cams is not None and CFG.plotly.camera_3d_coordinate.enable and CFG.plotly.ego_cam_traj.enable:
                ...
                build_cam_frustum_w_extr_ps(T_ego_world_cam, surface_mesh_id="ego_world_cam")
            ...
            if T_world_imus_w_open is not None and T_world_imus_w_close is not None and CFG.plotly.closed_slam_traj.enable and CFG.plotly.open_slam_traj.enable:
                ...
                build_cam_frustum_w_extr_ps(T_world_imu_w_close_traj, surface_mesh_id="imu_close_traj")
                T_world_imu_w_open_traj = T_world_imus_w_open[render_ind].copy()
                T_world_imu_w_open_traj[2, 3] += offset_z[render_ind]
                build_cam_frustum_w_extr_ps(T_world_imu_w_open_traj, surface_mesh_id="imu_close_traj")
            ...
            if Ts_world_cams is not None and CFG.plotly.camera_frustum.enable:
                for render_ind, Ts_world_cam in enumerate(Ts_world_cams):
                    ...
                    assert T == T, f"IMU and cam trajectory lengths do not match. IMU: {T}, Cam: {T}"
                    assert T == T_open, f"IMU and cam trajectory lengths do not match. IMU: {T_open}, Cam: {T}"

                    T_tmp = Ts_world_cam[render_ind].copy()
                    build_cam_frustum_w_extr_ps(T_tmp, surface_mesh_id="Ts_woldr_cam")

            if coco_world_3d_pts is not None and CFG.plotly.coco_kinematic_tree.enable:
                tmp_3d_pt = coco_world_3d_pts[render_ind][:NUM_EGOEXO4D_EGOPOSE_JNTS].copy()
                # tmp_3d_pt[:,2] += offset_z[ind]
                draw_coco_kinematic_tree_ps(tmp_3d_pt, coco_cfg=CFG.plotly.coco_kinematic_tree, curve_network_id=f"coco_world_3d_pts")
        
            if model_pred_coco_kpts is not None and CFG.plotly.model_pred_coco_kpts.enable:
                tmp_3d_pt = model_pred_coco_kpts[render_ind][:NUM_EGOEXO4D_EGOPOSE_JNTS].copy()
                # tmp_3d_pt[:,2] += offset_z[ind]
                draw_coco_kinematic_tree_ps(tmp_3d_pt, coco_cfg=CFG.plotly.model_pred_coco_kpts, curve_network_id=f"model_pred_coco_world_3d_pts")

            if model_gt_coco_kpts is not None and CFG.plotly.model_gt_coco_kpts.enable:
                tmp_3d_pt = model_gt_coco_kpts[render_ind][:NUM_EGOEXO4D_EGOPOSE_JNTS].copy()
                # tmp_3d_pt[:,2] += offset_z[ind]
                draw_coco_kinematic_tree_ps(tmp_3d_pt, coco_cfg=CFG.plotly.model_gt_coco_kpts, curve_network_id=f"model_gt_coco_world_3d_pts")
            ...
            if gen_vis_save_root_dir is not None:
                gen_vis_save_frame_path = osp.join(gen_vis_save_root_dir, "{:06d}.png".format(render_ind))
        ...
        if out_vid_dir is not None and gen_vis_save_root_dir is not None:
            if use_ffmpeg:
                images_to_video(img_folder=gen_vis_save_root_dir, output_vid_file=osp.join(out_vid_dir, "output.mp4"))
            else:
                images_to_video_w_imageio(img_folder=gen_vis_save_root_dir, output_vid_file=osp.join(out_vid_dir, "output.mp4"))


    # region
    def _plot_open3d(self, T_exo_world_cam_poses_w_anno: Optional[Dict[str,NDArray]] = None,\
        ...
        **kwargs):
        ...
        if T_exo_world_cam_poses_w_anno is not None and CFG.plotly.camera_frustum.enable:
            ...
            for i, exo_cam_name in enumerate(T_exo_world_cam_poses_w_anno.keys()):
                T_exo_world_cam_pose_w_anno = T_exo_world_cam_poses_w_anno[exo_cam_name].copy()
                T_exo_world_cam_pose_w_anno[2, 3] += mean_offset_z
                exo_cam_frustums[i] = build_cam_frustum_w_extr_open3d(T_exo_world_cam_pose_w_anno)
        ...
        if T_ego_world_cams is not None and CFG.plotly.camera_3d_coordinate.enable and CFG.plotly.ego_cam_traj.enable:
            T_ego = len(T_ego_world_cams)
            assert T_ego == T, f"Tracking timestamps and T_ego_world_cams do not match. Tracking: {T}, Ego: {T_ego}"
            _ = lambda : [None] * T_ego
            dynamic_traces = defaultdict(_)
            for i, frame_num in enumerate(T_ego_world_cams.keys()):
                ...
                dynamic_traces['ego_world_cam_x'][i], dynamic_traces['ego_world_cam_y'][i], dynamic_traces['ego_world_cam_z'][i] = draw_camera_pose(T_ego_world_cam)
            ...
            ego_cam_traj = go.Scatter3d(x=T_ego_world_cams[:, 0, 3], y=T_ego_world_cams[:, 1, 3], z=T_ego_world_cams[:, 2, 3],
                                mode="markers", marker={"size": CFG.plotly.ego_cam_traj.marker_size, "opacity": 0.8, "color":CFG.plotly.ego_cam_traj.color}, name="T_world_cam_traj",
                                hoverinfo='none', visible=False)
                                
            
            static_traces.append(ego_cam_traj)

        if T_world_imus_w_close is not None and T_world_imus_w_open is not None and CFG.plotly.closed_slam_traj.enable and CFG.plotly.open_slam_traj.enable:
            T_open, *_ = T_world_imus_w_open.shape
            T_close, *_ = T_world_imus_w_close.shape
            assert T_open == T_close == T, f"Open and close trajectory lengths do not match. Open: {T_open}, Close: {T_close}, T: {T}"
            ...
            for i in range(len(T_world_imus_w_close)):
                ...
                close_traj_cam_frustums[i] = build_cam_frustum_w_extr_open3d(T_world_imu_w_close_traj)
            ...
            for i in range(len(T_world_imus_w_open)):
                ...
                open_traj_cam_frustums[i] = build_cam_frustum_w_extr_open3d(T_world_imu_w_open_traj)
        ...
        if Ts_world_cams is not None and CFG.plotly.camera_frustum.enable:
            for ind, Ts_world_cam in enumerate(Ts_world_cams):
                ...
                for i in range(T):
                    ...
                    dynamic_traces[f"T_world_cam_{ind}_frustums"][i] = build_cam_frustum_w_extr_open3d(T_tmp)
                    dynamic_traces[f"T_world_cam_{ind}_x_frame"][i], dynamic_traces[f"T_world_cam_{ind}_y_frame"][i], dynamic_traces[f"T_world_cam_{ind}_z_frame"][i] = draw_camera_pose(T_tmp)

                Ts_world_cam_ = Ts_world_cam.copy()
                # Ts_world_cam_[:, 2, 3] += offset_z
                ego_cam_traj = go.Scatter3d(x=Ts_world_cam_[:, 0, 3], y=Ts_world_cam_[:, 1, 3], z=Ts_world_cam_[:, 2, 3],
                                    mode="markers", marker={"size": 2, "opacity": 0.8, "color": "blue"}, name="T_world_cam_traj",
                                    hoverinfo='none', visible=False)
                logger.info(
        ...
        coco_world_3d_kpts_key = "coco_world_3d_pts"
        coco_world_3d_pts = kwargs[coco_world_3d_kpts_key] if coco_world_3d_kpts_key in kwargs else None
        if coco_world_3d_pts is not None and CFG.plotly.coco_kinematic_tree.enable:
            assert T == len(coco_world_3d_pts), f"Tracking timestamps and coco world 3d points do not match. Tracking: {T}, Coco: {len(coco_world_3d_pts)}"
            for ind, coco_world_3d_pt in enumerate(coco_world_3d_pts):
                tmp_3d_pt = coco_world_3d_pt[:NUM_EGOEXO4D_EGOPOSE_JNTS].copy()
                # tmp_3d_pt[:,2] += offset_z[ind]
                coco_lines_vis = draw_coco_kinematic_tree_open3d(tmp_3d_pt, coco_cfg=CFG.plotly.coco_kinematic_tree)
                coco_lines_vis = {f"coco_world_3d_pts_{i}": coco_lines_vis[i] for i in range(len(coco_lines_vis))}
                for key, val in coco_lines_vis.items():
                    dynamic_traces[key][ind] = val
        
        # plot additional coco format world_3d_pts
        model_pred_coco_kpts = "model_pred_coco_kpts"
        model_pred_coco_kpts = kwargs[model_pred_coco_kpts] if model_pred_coco_kpts in kwargs else None
        if model_pred_coco_kpts is not None and CFG.plotly.model_pred_coco_kpts.enable:
            assert T == len(model_pred_coco_kpts), f"Tracking timestamps and coco world 3d points do not match. Tracking: {T}, Coco: {len(model_pred_coco_kpts)}"
            for ind, coco_world_3d_pt in enumerate(model_pred_coco_kpts):
                tmp_3d_pt = coco_world_3d_pt[:NUM_EGOEXO4D_EGOPOSE_JNTS].copy()
                # tmp_3d_pt[:,2] += offset_z[ind]
                coco_lines_vis = draw_coco_kinematic_tree_open3d(tmp_3d_pt, coco_cfg=CFG.plotly.model_pred_coco_kpts)
                coco_lines_vis = {f"model_pred_coco_world_3d_pts_{i}": coco_lines_vis[i] for i in range(len(coco_lines_vis))}
                for key, val in coco_lines_vis.items():
                    dynamic_traces[key][ind] = val

        # plot additional coco format world_3d_pts
        model_gt_coco_kpts = "model_gt_coco_kpts"
        model_gt_coco_kpts = kwargs[model_gt_coco_kpts] if model_gt_coco_kpts in kwargs else None
        if model_gt_coco_kpts is not None and CFG.plotly.model_gt_coco_kpts.enable:
            assert T == len(model_gt_coco_kpts), f"Tracking timestamps and coco world 3d points do not match. Tracking: {T}, Coco: {len(model_gt_coco_kpts)}"
            for ind, coco_world_3d_pt in enumerate(model_gt_coco_kpts):
                tmp_3d_pt = coco_world_3d_pt[:NUM_EGOEXO4D_EGOPOSE_JNTS].copy()
                # tmp_3d_pt[:,2] += offset_z[ind]
                coco_lines_vis = draw_coco_kinematic_tree_open3d(tmp_3d_pt, coco_cfg=CFG.plotly.model_gt_coco_kpts)
                coco_lines_vis = {f"model_gt_coco_world_3d_pts_{i}": coco_lines_vis[i] for i in range(len(coco_lines_vis))}
                for key, val in coco_lines_vis.items():
                    dynamic_traces[key][ind] = val
        ...
        if T_world_imus_w_close is not None and CFG.plotly.closed_slam_traj.enable:
            ...
            tmp_T[:, 2, 3] += offset_z
            imu_close_traj_points = np.array([tmp_T[:, 0, 3], tmp_T[:, 1, 3], tmp_T[:, 2, 3]]).T
            imu_close_traj_line_set = o3d.geometry.LineSet()
            imu_close_traj_line_set.points = o3d.utility.Vector3dVector(imu_close_traj_points)
            imu_close_traj_line_set.lines = o3d.utility.Vector2iVector([[i, i + 1] for i in range(len(imu_close_traj_points) - 1)])
            imu_close_traj_line_set.paint_uniform_color(CFG.plotly.closed_slam_traj.color)
        ...
        if T_world_imus_w_open is not None and CFG.plotly.open_slam_traj.enable:
            ...
            tmp_T[:, 2, 3] += offset_z
            imu_open_traj_points = np.array([tmp_T[:, 0, 3], tmp_T[:, 1, 3], tmp_T[:, 2, 3]]).T
            imu_open_traj_line_set = o3d.geometry.LineSet()
            imu_open_traj_line_set.points = o3d.utility.Vector3dVector(imu_open_traj_points)
            imu_open_traj_line_set.lines = o3d.utility.Vector2iVector([[i, i + 1] for i in range(len(imu_open_traj_points) - 1)])
        ...
        if point_clouds is not None and CFG.plotly.semidense_observations.enable:
            ...
            colors = (tmp_pts[:, 2] - np.min(tmp_pts[:, 2])) / (np.max(tmp_pts[:, 2]) - np.min(tmp_pts[:, 2]))  # Normalize z-coordinates for color mapping
            global_points.colors = o3d.utility.Vector3dVector(np.column_stack([colors, colors, colors]))  # Grayscale based on z
        ...
        for ele_key, traces in dynamic_traces.items():
            for i, trace in enumerate(traces):
                ...
                vis.add_geometry(trace)  # Add all geometries to the visualizer initially
    ...
    # region
    def _plot_plotly(self, T_exo_world_cam_poses_w_anno: Optional[Dict[str,NDArray]] = None,\
        T_exo_world_cam_w_gopro_calib: Optional[Dict[str,NDArray]] = None,
        T_ego_world_cams: Optional[Dict[str,NDArray]] = None, 
        T_world_imus_w_open: Optional[Dict[str,NDArray]] = None,
        T_world_imus_w_close: Optional[Dict[str,NDArray]] = None, 
        ...
        **kwargs):
        ...
        if T_exo_world_cam_poses_w_anno is not None and CFG.plotly.camera_frustum.enable:
            ...
            for i, exo_cam_name in enumerate(T_exo_world_cam_poses_w_anno.keys()):
                T_exo_world_cam_pose_w_anno = T_exo_world_cam_poses_w_anno[exo_cam_name].copy()
                T_exo_world_cam_pose_w_anno[2, 3] += mean_offset_z
                exo_cam_frustums[i] = build_cam_frustum_w_extr_plotly(T_exo_world_cam_pose_w_anno)
            static_traces.extend(exo_cam_frustums)
        ...
        if T_ego_world_cams is not None and CFG.plotly.camera_3d_coordinate.enable and CFG.plotly.ego_cam_traj.enable:
            ...
            for i, frame_num in enumerate(T_ego_world_cams.keys()):
                ...
                dynamic_traces['ego_world_cam_x'][i], dynamic_traces['ego_world_cam_y'][i], dynamic_traces['ego_world_cam_z'][i] = draw_camera_pose_plotly(T_ego_world_cam)
        ...
        if T_world_imus_w_close is not None and T_world_imus_w_open is not None and CFG.plotly.closed_slam_traj.enable and CFG.plotly.open_slam_traj.enable:
            ...
            for i in range(len(T_world_imus_w_close)):
                T_world_imu_w_close_traj = T_world_imus_w_close[i].copy()
                T_world_imu_w_close_traj[2, 3] += offset_z[i]
                close_traj_cam_frustums[i] = build_cam_frustum_w_extr_plotly(T_world_imu_w_close_traj)
            dynamic_traces["close_traj_cam_frustums"] = close_traj_cam_frustums
            ...
            for i in range(len(T_world_imus_w_open)):
                T_world_imu_w_open_traj = T_world_imus_w_open[i].copy()
                T_world_imu_w_open_traj[2, 3] += offset_z[i]
                open_traj_cam_frustums[i] = build_cam_frustum_w_extr_plotly(T_world_imu_w_open_traj)
            dynamic_traces["open_traj_cam_frustums"] = open_traj_cam_frustums
        ...
        if Ts_world_cams is not None and CFG.plotly.camera_frustum.enable:
            for ind, Ts_world_cam in enumerate(Ts_world_cams):
                ...
                for i in range(T):
                    T_tmp = Ts_world_cam[i].copy()
                    # T_tmp[2, 3] += offset_z[i]
                    dynamic_traces[f"T_world_cam_{ind}_frustums"][i] = build_cam_frustum_w_extr_plotly(T_tmp)
                    dynamic_traces[f"T_world_cam_{ind}_x_frame"][i], dynamic_traces[f"T_world_cam_{ind}_y_frame"][i], dynamic_traces[f"T_world_cam_{ind}_z_frame"][i] = draw_camera_pose(T_tmp)
        ...
        if coco_world_3d_pts is not None and CFG.plotly.coco_kinematic_tree.enable:
            ...
            for ind, coco_world_3d_pt in enumerate(coco_world_3d_pts):
                tmp_3d_pt = coco_world_3d_pt[:NUM_EGOEXO4D_EGOPOSE_JNTS].copy()
                # tmp_3d_pt[:,2] += offset_z[ind]
                coco_lines_vis = draw_coco_kinematic_tree_plotly(tmp_3d_pt, coco_cfg=CFG.plotly.coco_kinematic_tree)
                coco_lines_vis = {f"coco_world_3d_pts_{i}": coco_lines_vis[i] for i in range(len(coco_lines_vis))}
                for key, val in coco_lines_vis.items():
                    dynamic_traces[key][ind] = val
        ...
        if model_pred_coco_kpts is not None and CFG.plotly.model_pred_coco_kpts.enable:
            ...
            for ind, coco_world_3d_pt in enumerate(model_pred_coco_kpts):
                tmp_3d_pt = coco_world_3d_pt[:NUM_EGOEXO4D_EGOPOSE_JNTS].copy()
                # tmp_3d_pt[:,2] += offset_z[ind]
                coco_lines_vis = draw_coco_kinematic_tree_plotly(tmp_3d_pt, coco_cfg=CFG.plotly.model_pred_coco_kpts)
                coco_lines_vis = {f"model_pred_coco_world_3d_pts_{i}": coco_lines_vis[i] for i in range(len(coco_lines_vis))}
                for key, val in coco_lines_vis.items():
                    dynamic_traces[key][ind] = val
        ...
        if model_gt_coco_kpts is not None and CFG.plotly.model_gt_coco_kpts.enable:
            ...
            for ind, coco_world_3d_pt in enumerate(model_gt_coco_kpts):
                tmp_3d_pt = coco_world_3d_pt[:NUM_EGOEXO4D_EGOPOSE_JNTS].copy()
                # tmp_3d_pt[:,2] += offset_z[ind]
                coco_lines_vis = draw_coco_kinematic_tree_plotly(tmp_3d_pt, coco_cfg=CFG.plotly.model_gt_coco_kpts)
                coco_lines_vis = {f"model_gt_coco_world_3d_pts_{i}": coco_lines_vis[i] for i in range(len(coco_lines_vis))}
                for key, val in coco_lines_vis.items():
                    dynamic_traces[key][ind] = val
        ...
        if T_world_imus_w_close is not None and CFG.plotly.closed_slam_traj.enable:
            tmp_T = T_world_imus_w_close.copy()
            tmp_T[:, 2, 3] += offset_z
            imu_close_traj_go_item = go.Scatter3d(x=tmp_T[:, 0, 3], y=tmp_T[:, 1, 3],
                                        z=tmp_T[:, 2, 3], mode="markers",
                                        marker={"size": CFG.plotly.closed_slam_traj.marker_size, "opacity": CFG.plotly.closed_slam_traj.opacity, "color":CFG.plotly.closed_slam_traj.color},
                                        name="close_loop_trajecotory", hoverinfo='none', visible=False)
        ...
        if T_world_imus_w_open is not None and CFG.plotly.open_slam_traj.enable:
            tmp_T = T_world_imus_w_open.copy()
            tmp_T[:, 2, 3] += offset_z
            imu_open_traj_go_item = go.Scatter3d(x=tmp_T[:, 0, 3], y=tmp_T[:, 1, 3],
                                        z=tmp_T[:, 2, 3], mode="markers",
                                        marker={"size": CFG.plotly.open_slam_traj.marker_size, "opacity": CFG.plotly.open_slam_traj.opacity, "color":CFG.plotly.open_slam_traj.color},
                                        name="open_loop_trajectory", hoverinfo='none', visible=False)
        ...
        if point_clouds is not None and CFG.plotly.semidense_observations.enable:
            ...
            tmp_pts = point_clouds.copy()
            tmp_pts[:, 2] += mean_offset_z
            global_points = go.Scatter3d(x=tmp_pts[:, 0], y=tmp_pts[:, 1], z=tmp_pts[:, 2], mode="markers",
                marker={"size" : CFG.plotly.semidense_observations.size, "color": tmp_pts[:, 2], "cmin": -1.5, "cmax": 2, "colorscale":CFG.plotly.semidense_observations.colorscale,},
                name="Global Points", hoverinfo='none', visible=False)
            static_traces.append(global_points)
        ...
        for i in range(T):
            step = dict(
                method='update',
                args=[{'visible': [True] * len(static_traces) +  [time_step == i for traces in dynamic_traces.values() for time_step in range(T)]}],
        ...
        fig.update_layout(
            ...
            scene=dict(bgcolor='lightgray', dragmode='orbit', aspectmode='data', xaxis_visible=CFG.plotly.layout.x_visible,
                                      yaxis_visible=CFG.plotly.layout.y_visible, zaxis_visible=CFG.plotly.layout.z_visible),
        )
