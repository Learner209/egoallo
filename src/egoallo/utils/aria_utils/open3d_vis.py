...
def build_cam_frustum_w_extr(T_world_cam):
    ...
    mesh.triangles = o3d.utility.Vector3iVector([[0, 1, 2], [0, 2, 3], [0, 3, 4], [1, 2, 3], [1, 3, 4]])
...
def draw_coco_kinematic_tree(coco_kpts, coco_cfg) -> List[o3d.geometry.LineSet]:
    ...
    for p1, p2 in edges:
        ...
        line_set.paint_uniform_color(np.asarray(coco_cfg.color_rgb_line_body).astype(np.float32) if p1 > 4 else np.asarray(coco_cfg.color_rgb_line_head).astype(np.float32))
        line_sets.append(line_set)

    return line_sets


def draw_camera_pose(T: np.ndarray, colors: List[str] = ['red', 'green', 'blue']) -> List[o3d.geometry.LineSet]:
    """
    ...
    for axis, color in zip(axis_vectors, colors):
        ...
        line_set.paint_uniform_color(np.array(o3d.utility.Vector3dVector(np.array([color]))))
