...
def build_cam_frustum_w_extr(T_world_cam, surface_mesh_id=None):
    ...
    ps.register_surface_mesh(
        f"camera_frustum_{surface_mesh_id}",
        points_transformed,
        [[0, 1, 2], [0, 2, 3], [0, 3, 4], [1, 2, 3], [1, 3, 4]],
        color=(0.4, 0.7, 0.1),
    )  # Invisible color (RGBA)
...
def draw_coco_kinematic_tree(coco_kpts, coco_cfg, curve_network_id=None) -> None:
    ...
    for p1, p2 in edges:
        ...
        network = ps.register_curve_network(
            "coco_{}_{}_{}".format(p1, p2, curve_network_id),
            points,
            np.array([[0, 1]]),
            color=np.asarray(coco_cfg.color_rgb_line_body).astype(np.float64)
            if p1 > 4
