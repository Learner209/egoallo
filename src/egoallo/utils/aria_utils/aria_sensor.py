...
class AriaDataProvider:
    ...
    def get_image_by_index(self, stream_id, index):
        return self.provider.get_image_data_by_index(stream_id, index)[
            0
        ].to_numpy_array()

    def get_image_by_time_ns(
        self,
        stream_id,
        time_ns,
    ...
    def summarize_vrs(
        ...
        thumbnail = Image.new(
            "RGB",
            (int(width * sample_count / resize_ratio), int(height / resize_ratio)),
        )
        current_width = 0
    ...
    def get_sensor_data_w_capture_time(self):
        ...
        for sensor_data in iterator:
            ...
            sensor_data.get_time_ns(TimeDomain.TIME_CODE)
            print(
