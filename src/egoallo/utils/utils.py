...
def make_source_code_snapshot(log_dir, logger):
    if not osp.exists(osp.join(log_dir, "source")):
        logger.info("Backuping source....")
        ...
        exclusion = " ".join(["--exclude " + f'"{a}"' for a in exclusions])
        cmd = cmd + " " + exclusion
        ...
        logger.info("Backuping source done!")
