...
def main(opt: Joints2SmplFittingConfig):
    ...
    smplmodel = smplx.create(joints2smpl_config.SMPL_MODEL_DIR, 
                            model_type="smpl", gender="neutral", ext="pkl",
                            batch_size=opt.batch_size).to(device)
    ...
    # # #-------------initialize SMPLify
    smplify_model = smplify.SMPLify3D(smplxmodel=smplmodel,
    ...
    for idx in range(num_seqs):
        ...
        new_opt_cam_t, new_opt_joint_loss = smplify_model(
                                                    pred_pose.detach(),
                                                    pred_betas.detach(),
                                                    pred_cam_t.detach(),
                                                    keypoints_3d,
...
def joints2smpl_fit_seq(opt: Joints2SmplFittingConfig, body_model: fncsmpl.SmplhModel, batch_size: int, joints3d: Float[Tensor, "batch_size 22 3"], output_dir: Path):
    ...
    pred_root_position = torch.zeros(batch_size, 3).to(device)
