...
def do_guidance_optimization(
    ...
    quats, debug_info = _optimize_vmapped(
        ...
        T_world_root=cast(jax.Array, T_world_root.mean(axis=tuple(range(len(T_world_root.shape)-2))).numpy(force=True)), # (*B, T, 7)
        betas=cast(jax.Array, traj.betas.numpy(force=True)), # (*B, T, 16)
        body_rotmats=cast(jax.Array, traj.body_rotmats.numpy(force=True)), # (*B, T, 21, 3, 3)
        hand_rotmats=cast(jax.Array, traj.hand_rotmats.numpy(force=True)), # (*B, T, 30, 3, 3)
        contacts=cast(jax.Array, traj.contacts[..., :, 1:].numpy(force=True)), # (*B, T, 22)
...
def _optimize(
    ...
    def angle_prior_cost(
        ...
        assert isinstance(local_quats_aa, Float[Tensor, "69"])
        ...
        return guidance_params.angle_prior_weight * jnp.sum(
            jnp.exp(joint_angles * signs) ** 2,
            axis=-1
        )
