...
class MotionProcessor:
    ...
    def process_floor_and_contacts(
        ...
        for side in ["left", "right"]:
            ...
            contacts[:, toe_idx] = (toe_vel < self.contact_vel_thresh) & (
            ...
            contacts[:, ankle_idx] = (ankle_vel < self.contact_vel_thresh) & (
                joints[:, ankle_idx, 2]
                < floor_height + self.contact_ankle_height_thresh
            )

            # Knee contacts
            knee_idx = joint_indices[f"{side}_knee"]
            knee_vel = self.compute_joint_velocity(joints[:, knee_idx])
            contacts[:, knee_idx] = (knee_vel < self.contact_vel_thresh) & (
            ...
            contacts[:, elbow_idx] = (elbow_vel < self.contact_vel_thresh) & (
                joints[:, elbow_idx, 2]
                < floor_height + self.contact_ankle_height_thresh
            )
            ...
            contacts[:, wrist_idx] = (wrist_vel < self.contact_vel_thresh) & (
                joints[:, wrist_idx, 2]
                < floor_height + self.contact_ankle_height_thresh
            )
            ...
            contacts[:, foot_idx] = (foot_vel < self.contact_vel_thresh) & (
    ...
    def detect_floor_height(
        ...
        if len(static_heights) > MAX_SAMPLES:
            ...
            weights = 1.0 / (
                hist[np.digitize(static_heights, bin_edges[1:], right=True)] + 1
            )
    ...
    def compute_joint_velocity(
        positions: Float[np.ndarray, "*batch 3"],
    ...
    def compute_angular_velocity(
        ...
        ang_vel = np.pad(
            ang_vel,
            ((1, 1), *[(0, 0) for _ in range(ang_vel.ndim - 2)], (0, 0)),
            mode="edge",
        )  # (N, ..., 3)
